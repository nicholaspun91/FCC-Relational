...
echo "$($PSQL "select name from teams inner join games on teams.team_id=games.opponent_id where year=2014 and round='Eighth-Final' UNION select name from teams inner join games on teams.team_id=games.winner_id where year=2014 and round='Eighth-Final'")"
